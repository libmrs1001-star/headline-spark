...
def get_ai_analysis(title: str) -> str:
    """
    使用 DeepSeek API 分析标题（使用 requests 库）
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_event("ai_analysis_error", title, description_provided=f"API错误: {response.status_code} - {error_info}")
...
def get_ai_ideas(description: str = None) -> List[str]:
    """
    使用 DeepSeek API 生成标题创意（使用 requests 库）
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # API 返回错误
            error_info = response.json() if response.content else {"error": "Unknown error"}
            log_event("ai_ideas_error", description_provided=f"API错误: {response.status_code} - {error_info}")
            return [
                "抱歉，AI 创意生成服务当前不可用",
                "请检查网络连接或稍后再试",
                "如需帮助，请联系技术支持"
